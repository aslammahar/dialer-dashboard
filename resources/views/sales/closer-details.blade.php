...
@section('title', 'Complete Closer Reports with All Status Columns and Carrier Breakdown')
...
<style>
    ...
    .detailed-table th.closer-name-cell {
        position: sticky !important;
        left: 0 !important;
    ...
    .client-subheader {
        ...
        border-right: 1px solid #059669 !important;
    }
    ...
    @media (max-width: 1600px) {
        ...
        .closer-name-cell { min-width: 150px !important; max-width: 150px !important; }
...
<div class="detailed-reports-dashboard">
    ...
    <div class="main-container">
        ...
        <div class="header-section">
            <h1 class="main-title">📊 Complete Client Analytics Dashboard</h1>
            <p class="subtitle">
                Comprehensive closer performance with status and carrier breakdown
            ...
            @if(isset($allClients) && $allClients->count() > 0)
                <div class="client-count">
                    ...
                    👥 {{ isset($closerReports) ? $closerReports->count() : 0 }} Closers |
                    📅 {{ $period }} | 
            ...
            <div class="filter-container" style="margin-top: 2rem;">
                <a href="{{ route('closer.detailed.report', ['filter' => 'daily']) }}" 
                   class="filter-btn {{ $filter === 'daily' ? 'active' : '' }}">
                    📅 Daily Analysis
                </a>
                <a href="{{ route('closer.detailed.report', ['filter' => 'weekly']) }}" 
                   class="filter-btn {{ $filter === 'weekly' ? 'active' : '' }}">
                    📊 Weekly Breakdown
                </a>
                <a href="{{ route('closer.detailed.report', ['filter' => 'monthly']) }}" 
        ...
        <div class="action-bar">
            <div>
                <h2>📈 Complete Performance Report - {{ $period }}</h2>
            ...
            <div style="display: flex; gap: 1rem; align-items: center; flex-wrap: wrap;">
                ...
                <input type="text" id="closerSearch" placeholder="🔍 Search closers..." 
                ...
                <a href="{{ route('closer.reports') }}" class="btn btn-secondary">
                    ← Back to Summary
                </a>
        ...
        <div class="detailed-table-container">
            @if(isset($closerReports) && $closerReports->count() > 0 && isset($allClients) && $allClients->count() > 0)
                <div class="table-wrapper" id="tableWrapper">
                    ...
                    <table class="detailed-table" id="dynamicTable">
                        <thead>
                            <!-- Main Header Row -->
                            <tr>
                                <th rowspan="2" class="metric-cell">👤 Closer Name</th>
                        ...
                        <tbody id="tableBody">
                            @foreach($closerReports as $index => $report)
                                <tr class="data-row" data-closer="{{ strtolower($report->closer_name) }}" style="animation-delay: {{ $index * 0.05 }}s;">
                                    <!-- Fixed Closer Info Columns -->
                                    <td class="closer-name-cell">
                                        <div style="display: flex; align-items: center; gap: 0.5rem;">
                                            <div style="width: 8px; height: 8px; border-radius: 50%; background: linear-gradient(135deg, #059669, #10b981);"></div>
                                            <div>
                                                <div style="font-weight: 700;">{{ $report->closer_name }}</div>
                                            </div>
                                        </div>
                                    </td>
                            ...
                            @php
                                $total = [
                                    'submissions' => $closerReports->sum('total_submissions'),
                                    'approved' => $closerReports->sum('approved_count'),
                                    'pending' => $closerReports->sum('pending_count'),
                                    'rejected' => $closerReports->sum('rejected_count'),
                                    'level' => $closerReports->sum('level_count'),
                                    'gi' => $closerReports->sum('gi_count'),
                                    'yearly_premium' => $closerReports->sum('yearly_premium'),
                                    'avg_premium' => $closerReports->where('avg_premium', '>', 0)->avg('avg_premium'),
                                ];
            ...
            @else
                <div class="empty-state">
                    ...
                    <p style="margin: 0 0 2rem 0; font-size: 1rem;">
                        @if(!isset($closerReports) || $closerReports->count() === 0)
                            No closer performance data found for {{ $period }}.
        ...
        <!-- Statistics Summary -->
        @if(isset($closerReports) && $closerReports->count() > 0 && isset($allClients))
            <div class="stats-grid">
                <div class="stat-card">
                    <h4>👥 Active Closers</h4>
                    <div class="stat-value">{{ $closerReports->count() }}</div>
                    <div class="stat-label">Total Closers Tracked</div>
                </div>
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-value">{{ ($closerReports->count() ?? 0) * ($allClients->count() ?? 0) * (8 + count($activeCarriers)) }}</div>
...
console.log('🚀 Complete Client Analytics Dashboard Loaded');
console.log('Period:', '{{ $period }}');
console.log('Total Closers:', {{ isset($closerReports) ? $closerReports->count() : 0 }});
...
function setupSearchFunctionality() {
    const searchInput = document.getElementById('closerSearch');
    ...
    searchInput.addEventListener('input', debounce(function() {
        ...
        rows.forEach(row => {
            const closerName = row.dataset.closer || '';
            const isVisible = closerName.includes(searchTerm);
            ...
            if (table) {
                ...
                columnCells.forEach(colCell => {
                    if (colCell !== this && colCell.textContent.trim()) {
                        colCell.style.background = 'rgba(16, 185, 129, 0.05)';
                    }
                });
            }
        });
...
function setupResponsiveTable() {
    ...
    console.log(`📏 Table configured: ${clientCount} clients, ${8 + carrierCount} cols each, ${totalWidth}px total width`);
}
...
function exportToExcel() {
    ...
    console.log('📊 Starting Excel export with status and carrier columns...');
    ...
    try {
        ...
        for (let c = 0; c <= range.e.c; c++) {
            if (c === 0) colWidths.push({ width: 25 }); // Closer name
        ...
        XLSX.utils.book_append_sheet(wb, ws, 'Complete Closer Performance');
        
        const now = new Date();
        const dateStr = now.toISOString().split('T')[0];
        const filename = `complete_closer_performance_with_carriers_{{ $filter }}_${dateStr}.xlsx`;
        
        XLSX.writeFile(wb, filename);
        
        console.log(`✅ Excel file exported: ${filename}`);
        showNotification('✅ Excel file with status and carrier columns downloaded!', 'success');
...
console.log('✨ Complete Client Analytics Dashboard Fully Loaded! 🎯');
console.log('📊 Features: Status tracking, carrier breakdown');
