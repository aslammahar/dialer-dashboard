...
@section('page-title', 'Retention Carrier Wise Report')
...
<div class="cw-wrap">
   <div class="cw-head">
    <div class="cw-title">📊 Retention Carrier Wise</div>
    <div style="display:flex;gap:8px;align-items:center;flex-wrap:wrap">
        <a href="{{ route('retention.reports.carrier-wise', ['range' => 'daily']) }}" class="cw-apply" style="{{ $range == 'daily' ? '' : 'background:var(--cw-surface);color:var(--cw-text-sec);border:1px solid var(--cw-border)' }};text-decoration:none">Daily</a>
        <a href="{{ route('retention.reports.carrier-wise', ['range' => 'weekly']) }}" class="cw-apply" style="{{ $range == 'weekly' ? '' : 'background:var(--cw-surface);color:var(--cw-text-sec);border:1px solid var(--cw-border)' }};text-decoration:none">Weekly</a>
        <a href="{{ route('retention.reports.carrier-wise', ['range' => 'monthly']) }}" class="cw-apply" style="{{ $range == 'monthly' ? '' : 'background:var(--cw-surface);color:var(--cw-text-sec);border:1px solid var(--cw-border)' }};text-decoration:none">Monthly</a>

        @if($range == 'monthly')
        <form method="GET" action="{{ route('retention.reports.carrier-wise') }}" style="display:flex;gap:8px">
...
<div class="cw-box">
    <div style="font-family:var(--cw-font-display);font-weight:700;font-size:14px;margin-bottom:12px">Detailed Closer × Carrier Breakdown</div>
        ...
        <table class="cw-table">
            <thead>
                <tr>
                    ...
                    @endforeach
                    <!-- Divider for UI -->
                    <th style="border-left:1px solid var(--cw-border); padding-left:15px">---</th>
            ...
            <tbody>
                @forelse($board as $row)
                    <tr>
                        ...
                        <td style="border-left:1px solid var(--cw-border)"></td>
                        @foreach($clients as $client)
                            <td>{{ $row['clients'][$client] ?? 0 }}</td>
                        @endforeach
                    </tr>
                @empty
                    <tr><td colspan="{{ 3 + count($carriers) + count($clients) }}" style="text-align:center;color:var(--cw-text-muted);padding:20px">No data for this selection.</td></tr>
                @endforelse
            </tbody>
            <tfoot>
                <tr>
                    ...
                    @foreach($carriers as $carrier)
                        <td>{{ $totals[$carrier] ?? 0 }}</td>
                    @endforeach
                    
                    <td style="border-left:1px solid var(--cw-border)"></td>
                    @foreach($clients as $client)
                        <td>{{ $totals["client_$client"] ?? 0 }}</td>
                    @endforeach
                </tr>
            </tfoot>
        </table>
