...
<div class="container mt-4">
    <div class="card">
        <div class="card-header justify-content-between">
            ...
            @can(abilities: 'closed call records')
            <a href="{{ url('/dialer-edit', $closedCall->id) }}" class="btn btn-info btn-sm">Dialer Edit</a>
                   
            @endcan
            

            @can('closed call records')
            <a href="{{ url('/edit-closed-calls', parameters: $closedCall->id) }}" class="btn btn-info btn-sm">Admin Edit</a>
                   
            @endcan

           @if(auth()->user()->can('own client policy edit') || auth()->user()->email === config('variables.master_company_email'))
                @if($closedCall->clients_id)
                    <a href="{{ route('client_policy.edit', $closedCall->id) }}" class="btn btn-info btn-sm">
                        <i class="fas fa-user-edit me-1"></i>Own Client Edit
                    </a>
                @else
                    <button class="btn btn-secondary btn-sm" disabled>
                        <i class="fas fa-user-edit me-1"></i>Not assigned yet
                    </button>
                @endif
            @endif
        </div>
            ...
            <div class="row">
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        @if($closedCall->assigned_history)
                            <div class="p-3 bg-light border rounded">
                                {!! nl2br(e($closedCall->assigned_history)) !!}
                            </div>
                        @else
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        @if($closedCall->closer_assigned_history)
                            <div class="p-3 bg-light border rounded" style="max-height: 200px; overflow-y: auto;">
                                {!! nl2br(e($closedCall->closer_assigned_history)) !!}
                            </div>
                        @else
                            <div class="p-3 bg-light border rounded text-muted fst-italic">
                                No closer assignment history available
                            </div>
                        @endif
                    </div>
                </div>
                    ...
                    <p><strong>Client Policy Updated by:</strong> {{ $closedCall->updater->name ?? 'N/A' }}</p>
