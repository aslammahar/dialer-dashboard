...
return new class extends Migration
{
    ...
    {
        Schema::create('retention_sales_entries', function (Blueprint $table) {
            $table->id();
            ...
            $table->enum('retention_type', ['rewrite', 'fixed', 'correspondence', 'new_policy'])->default('rewrite');
            $table->enum('status', ['approved', 'pending'])->default('approved');
            $table->enum('sale_type', ['level', 'gi'])->nullable();
    ...
    {
        Schema::dropIfExists('retention_sales_entries');
    }
};
