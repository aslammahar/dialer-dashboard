    ...
    public function allBugList($view)
    {
          ...
          elseif(Auth::user()->type != 'company'){
            if(\Auth::user()->type == 'client'){
              ...
              $escapedUserId = DB::connection()->getPdo()->quote((string)\Auth::user()->id);
              $bugs = Bug::where('created_by',\Auth::user()->creatorId())->whereRaw("find_in_set(" . $escapedUserId . ",assign_to)")->get();
            }
          }
          if($view == 'list'){
        ...
        if($request->ajax() && $request->has('view') && $request->has('sort'))
        {
            ...
            if(\Auth::user()->type != 'company'){
              if(\Auth::user()->type == 'client'){
                ...
                // 🔒 SECURITY: Escape user ID to prevent SQL injection
                $escapedUserId = DB::connection()->getPdo()->quote((string)$usr->id);
                $tasks->whereRaw("find_in_set(" . $escapedUserId . ",assign_to)");
              }
            }
            else{
              $tasks->where('created_by',\Auth::user()->creatorId());
        ...
        if($usr->type != 'admin')
        {
            ...
            elseif(\Auth::user()->type != 'company'){
              if(\Auth::user()->type == 'client'){
                ...
                // 🔒 SECURITY: Escape user ID to prevent SQL injection
                $escapedUserId = DB::connection()->getPdo()->quote((string)\Auth::user()->id);
                $tasks = ProjectTask::whereIn('project_id', $user_projects)->whereRaw("find_in_set(" . $escapedUserId . ",assign_to)");
              }
            }
            if(\Auth::user()->type  == 'client'){
              if($task_by == 'all')
