...
class TaxController extends Controller
{
    ...
    public function destroy(Tax $tax)
    {
        if(\Auth::user()->can('delete constant tax'))
        {
            if($tax->created_by == \Auth::user()->creatorId())
            {
                // 🔒 SECURITY: Escape tax ID to prevent SQL injection
                $escapedTaxId = DB::connection()->getPdo()->quote((string)$tax->id);
                $proposalData = ProposalProduct::whereRaw("find_in_set(" . $escapedTaxId . ",tax)")->first();
                $billData     = BillProduct::whereRaw("find_in_set(" . $escapedTaxId . ",tax)")->first();
                $invoiceData  = InvoiceProduct::whereRaw("find_in_set(" . $escapedTaxId . ",tax)")->first();
