...
//          current month 
    ...
    public function store(Request $request)
    {
        ...
        $validatePayslip = PaySlip::where('salary_month', '=', $formate_month_year)
                                   ->where('created_by', \Auth::user()->creatorId())
                                   ->pluck('employee_id');
    
        $payslip_employee_count = Employee::where('created_by', \Auth::user()->creatorId())
                                           ->where('company_doj', '<=', date($year . '-' . $month . '-t'))
                                           ->count();
    
        if ($payslip_employee_count > count($validatePayslip)) {
            $employees = Employee::where('created_by', \Auth::user()->creatorId())
                                 ->where('company_doj', '<=', date($year . '-' . $month . '-t'))
                                 ->whereNotIn('id', $validatePayslip)
                                 ->get();
            ...
            foreach ($employees as $employee) {
                // Get the user_id from Employee and check if the user is active
                $user = User::where('id', $employee->user_id)->first();
                if ($user && $user->is_active == 0) {
                    continue; // Skip this employee if the user is inactive
                }
    
                // Generate payslip for active users
                ...
                if (isset($setting['payslip_notification']) && $setting['payslip_notification'] == 1) {
                    $msg = __("Payslip generated for") . ' ' . $formate_month_year . '.';
