...
//                ClientPermission::where('deal_id', '=', $deal->id)->delete();
    ...
    public function priorities($id)
    {
        if(\Auth::user()->can('edit deal'))
        {
            ...
            if($deal->created_by == \Auth::user()->ownerId())
            {
                $priorities   = Priority::where('pipeline_id', '=', $deal->pipeline_id)->where('created_by', \Auth::user()->creatorId())->get();
                $selected = $deal->priorities();
                if($selected)
                {
                ...
                return view('deals.priorities', compact('deal', 'priorities', 'selected'));
            }
            else
            {
                return response()->json(['error' => __('Permission Denied.')], 401);
    ...
    public function priorityStore($id, Request $request)
    {
        if(\Auth::user()->can('edit deal'))
        {
            ...
            if($deal->created_by == \Auth::user()->ownerId())
            {
                ...
                {
                    $deal->priorities = implode(',', $request->priorities);
                }
                else
                {
                    $deal->priorities = $request->priorities;
                }
                $deal->save();
    ...
    public function groups($id)
    {
        if(\Auth::user()->can('edit deal'))
        {
            ...
            if($deal->created_by == \Auth::user()->ownerId())
            {
                $groups   = Group::where('pipeline_id', '=', $deal->pipeline_id)->where('created_by', \Auth::user()->creatorId())->get();
                $selected = $deal->groups();
                if($selected)
                {
                    $selected = $selected->pluck('name', 'id')->toArray();
                }
                ...
                return view('deals.groups', compact('deal', 'groups', 'selected'));
            }
            else
            {
                return response()->json(['error' => __('Permission Denied.')], 401);
    ...
    public function groupStore($id, Request $request)
    {
        if(\Auth::user()->can('edit deal'))
        {
            ...
            if($deal->created_by == \Auth::user()->ownerId())
            {
                if($request->groups)
                {
                    $deal->groups = implode(',', $request->groups);
                }
                else
                {
                    $deal->groups = $request->groups;
                }
                $deal->save();
