...
class ApiController extends Controller
{
    ...
    // get closed call where client id is 4
   

    public function handleEvent(Request $request)
    {
        //  Log::info('Hikvision webhook hit', [
        //     'all' => $request->all(),
        ...
        if ($eventLog) {
            ...
            if (json_last_error() === JSON_ERROR_NONE && isset($data['AccessControllerEvent'])) {
                $employeeId = $data['AccessControllerEvent']['employeeNoString'] ?? null;
                $employeeName = $data['AccessControllerEvent']['name'] ?? null;
                $status = $data['AccessControllerEvent']['attendanceStatus'] ?? null;
                $dateTime = $data['dateTime'] ?? now()->toDateTimeString();

                // create attendance only if $employeeId is not null
                if ($employeeId) {
                    $attendance = HikVisionAttendance::create([
                    ...
                    \Log::info('Attendance stored', ['id' => $attendance->id]);
                }

            } else {
                \Log::error('Invalid Hikvision Event JSON', ['event_log' => $eventLog]);
            }
        } else {
            \Log::warning('No event_log field in Hikvision request');
        }
