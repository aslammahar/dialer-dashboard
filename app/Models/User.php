...
class User extends Authenticatable
{
    ...
    {
        // NOTE:
        // We intentionally DO NOT add a global center scope on User.
        // Doing so can break authentication (Laravel loads the authenticated user via User queries),
        // and can cause recursion / memory exhaustion on boot.
        //
        // Use `User::query()->forCurrentCenter()` explicitly where needed.

        // Auto-assign center_id on new users (unless bypass user).
        static::creating(function (User $user) {
            ...
            if ($authUser->canBypassCenterScope()) {
                // Admins can create users for any center (keep whatever was set).
                return;
            }

            // Force new user's center_id to the creator's center_id.
            $user->center_id = $authUser->center_id;
        });
    }

    /**
     * User types allowed to see cross-center data.
     * Keep this consistent with your other multi-tenant scopes.
     */
    ...
    {
        return in_array($this->type, self::CENTER_BYPASS_TYPES, true);
    }

    /**
     * Scope users to the authenticated user's center.
     * Use this in controllers when listing users for QA/assignment/etc.
     */
    public function scopeForCurrentCenter(Builder $query): Builder
    {
        if (app()->runningInConsole()) {
            return $query;
        }
        ...
        if (!empty($authUser->center_id)) {
            return $query->where('center_id', $authUser->center_id);
        }

        // If user has no center_id, return no rows rather than leaking data.
    ...
    public static function show_hrm()
    {
        ...
        if (!empty($user->plan)) {
            ...
            return $plan ? $plan->hrm : '';
        }

        // Company/super admin with no plan row: still show HRM in UI (matches accounts where type was set without plan).
        if ($user_type == 'company' || $user_type == 'super admin') {
            return 1;
        }
 ...
 days I have&nbsp; {leave_status} your leave request for {leave_reason}.</span><br></p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;"><span style="font-family: var(--bs-body-font-family); font-weight: var(--bs-body-font-weight); text-align: var(--bs-body-text-align);">We request you to complete all your pending work or any other important issue so that the company does not face any any loss or problem during your absence. We appreciate your thoughtfulness to inform us well in advance.</span></p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;">Feel free to reach out if you have any questions.</p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;">Thank You,</p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;">Regards,</p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;">HR Department,</p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;">{app_name}</p><p segoe="" ui",="" arial;="" font-size:="" 14px;"="" style="line-height: 28px;">{app_url}</p><p></p>',
    ...
    public function voiceLeads()
    {
        return $this->hasMany(VoiceQALead::class, 'agent_id');
    }

    public function qaleadsvoice()
    {
        return $this->hasOne(QaLeadsVoice::class, 'user_email', 'email');
    }

    // avatarlead relations
    public function avatarLeads()
    {
        return $this->hasMany(AvatarLead::class, 'QaPersonId');
    }



    public function avatarQALeads()
    {
        return $this->hasMany(AvatarQALead::class, 'agent_email', 'email');
    }
    ...
    {
        return $this->belongsToMany(Team::class, 'team_agent', 'agent_id', 'team_id');
    }


    // relation with closedCalls table

    public function closedCalls()
    {
        return $this->hasMany(ClosedCall::class, 'closer_id');
    }
 ...
 public function closedCallsAsCloser()
    {
        return $this->hasMany(ClosedCall::class, 'closer_id');
    }
    ...
    public function vendor()
    {
        ...
        public function closerTeam(): \Illuminate\Database\Eloquent\Relations\BelongsToMany
{
    return $this->belongsToMany(CloserTeam::class, 'closer_team_members', 'user_id', 'closer_team_id')
                ->withPivot('joined_at')
                ->withTimestamps();
}

/**
 * Get the closer team member pivot record
 */
public function closerTeamMember(): \Illuminate\Database\Eloquent\Relations\HasOne
{
    return $this->hasOne(CloserTeamMember::class);
}

/**
 * Check if user is in any closer team
 */
public function isInCloserTeam(): bool
{
    return $this->closerTeamMember()->exists();
}

/**
 * Get current closer team
 */
public function getCurrentCloserTeam(): ?CloserTeam
{
    return $this->closerTeam()->first();
}

/**
 * Scope to get only closers
 */
public function scopeClosers($query)
{
    return $query->where('type', ['Closer', 'Outsourcing']);
}

/**
 * Scope to get closers not in any team
 */
public function scopeClosersWithoutTeam($query)
{
    return $query->whereIn('type', ['Closer', 'Outsourcing'])
...
{
    return $this->hasMany(UserBankDetail::class)->orderBy('priority');
}

public function salaryDepartments()
{
    return $this->belongsToMany(SalaryDepartment::class, 'salary_department_user', 'user_id', 'salary_department_id');
}
public function salaryStructures()
    {
        return $this->hasMany(\App\Models\SalaryStructure::class, 'user_id');
    }

 public function supports()
    {
        return $this->belongsToMany(DepartmentSupport::class, 'department_support_user');
    }
